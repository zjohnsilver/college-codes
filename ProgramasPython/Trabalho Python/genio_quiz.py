...
def main():
	...
	def evaluate(event):
		data = entrada.get()
		if (int(data) == fatorial(random_fatorial)):
			show_frame(frames['fv'])
		else:
			show_frame(frames['ff'])

	try:
		arquivo = open("perguntas.txt", "r")
		perguntas = arquivo.read().split("\n")
	except:
		exit("Arquivo de perguntas não existe, por isso o programa não pode ser aberto.")
	...
	for x in range(1, 18):
		...
		if x>=2:
			...
			if valor_da_subframe == 9:
				...
				Label(subFrame4, image = imagem).pack()
				subFrame4.pack()	
	...
	numero_pergunta = Button (sub_frames['s57'], text = '15 - ', height = '1', width = '3', font = ("Verdana", "13"), command = lambda: show_frame(frames['f17']), relief = FLAT).pack(side = "left")
	...
	#16 Pergunta
	random_fatorial = randint(1,6)
	Label(sub_frames['s%i'%(x)], text = "%s!  =  "%(random_fatorial), font = ("Verdana", "20")).pack(side = "left")
	entrada = Entry(sub_frames['s%i'%(x)], font = ("Verdana", "20", "italic"), relief = FLAT)
	entrada.bind("<Return>", evaluate)
	entrada.pack()
